...
export class UserService {
    ...
    async findUserDeatils(id: number){
        ...
        const result = await this.userRepository.query("SELECT hrm_departments.dept_name from users left JOIN hrm_jobtitle_user on users.id = hrm_jobtitle_user.user_id left join hrm_jobtitles on hrm_jobtitle_user.job_title_id = hrm_jobtitles.id LEFT JOIN hrm_departments on hrm_jobtitles.departments_id = hrm_departments.id WHERE users.id = '"+id+"';");
        console.log('===RESULTS===');
